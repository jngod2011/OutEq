...
//Rstar = R*(1+rhobhatbar)/(1+cstar);
  ...
  L = (w*mU/ksi)^(1/fi);
  C + (1+cstar+kappa0b*(exp(kappa1b*phib)-1)*phib+kappa0e*(exp(kappa1e*phie)-1)*phie+kappa0_sub*((exp(kappa1_sub*(sub-1))-1)*(sub-1)))*I = Y; // 10 
 ...
 //% phib=0;
  //%kappa0e*(exp(kappa1e*phie)-1) + kappa0e*kappa1e*exp(kappa1e*phie)*phie = 1 - etaw/etab;
    kappa0b*(exp(kappa1b*phib)-1) + kappa0b*kappa1b*exp(kappa1b*phib)*phib = sub*(pH/dp)*cstar*(Qbhat - 1)/((pH/dp)*cstar*Qbhat + phib/Qbhat);
...
planner_objective (C^(1-sigma))/(1-sigma)-(ksi/(1+fi))*L^(1+fi);
